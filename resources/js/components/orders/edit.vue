                ...
                <div class="row mb-3 align-items-center">
                    <div class="co-12 col-md-3">
                        <div class="input-group mb-3">
                            ...
                            <input id="username" name="username" class="form-control text-capitalize" disabled :value="current"/> 
                        </div>
                    </div>
...
data(){
    ...
    methods:{
        ...
        submit(){
            ...
            this.data["username"] = this.current
            if(this.items && this.total > 0 && this.current)
                {
                    this.items.every(item => {
                        if(item['qty'] > 0){
                            ...
                            alert("Enter Valid value of" + item['name'])
                            return false
                        }
                    });
                    if(this.send)
                    {
                        axios.put(`http://localhost/NewPh/public/orders/${this.$route.params.id}`,this.data)
        ...
        edit(id) {
            axios.get(`http://localhost/NewPh/public/orders/${id}/edit`)
                .then((res) => {
                    this.order=res.data;
                    this.order["drugs"].forEach(item => {
                        this.drugindex = this.drugs.findIndex( drug =>drug.name === item.drug_name)
                        this.items.push({
                            name:this.drugs[this.drugindex].name,
                            qty:item.qty,
                            type:this.drugs[this.drugindex].type,
                            unit:item.unit,
                            smallprice:this.drugs[this.drugindex].smallprice,
                            packet:this.drugs[this.drugindex].packet,
                            disc:item.disc,
                            olddisc:this.drugs[this.drugindex].disc,
                            avilable:this.drugs[this.drugindex].avilablesunit,
                            showav:function(a,t,p){
                                if(t==="Syrp")
                                    return a;
                                else
                            ...
                            this.filter(this.drugs[this.drugindex].name)
                            this.updatetotal()
                            this.drugindex=''
                        })
                        this.current=this.order["user_name"] 
                }
            );
        },
    }
